// lib/core/localization/app_strings.dart

class AppStrings {
  // â”€â”€ App â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const appName = 'QualifAI';
  static const appNameAr = 'ÙƒÙŠÙˆÙ„ÙŠÙ AI';
  static const tagline = 'Ù†Ø¸Ø§Ù… Ø§Ù„Ø¬ÙˆØ¯Ø© ÙˆØ§Ù„Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„Ø£ÙƒØ§Ø¯ÙŠÙ…ÙŠ';
  static const taglineEn = 'Academic Quality & Accreditation System';

  // â”€â”€ Auth â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const welcome = 'Ù…Ø±Ø­Ø¨Ø§Ù‹ Ø¨Ùƒ';
  static const welcomeEn = 'Welcome Back';
  static const username = 'Ø§Ø³Ù… Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…';
  static const usernameEn = 'Username';
  static const email = 'Ø§Ù„Ø¨Ø±ÙŠØ¯ Ø§Ù„Ø¥Ù„ÙƒØªØ±ÙˆÙ†ÙŠ';
  static const emailEn = 'Email';
  static const password = 'ÙƒÙ„Ù…Ø© Ø§Ù„Ù…Ø±ÙˆØ±';
  static const passwordEn = 'Password';
  static const login = 'ØªØ³Ø¬ÙŠÙ„ Ø§Ù„Ø¯Ø®ÙˆÙ„';
  static const loginEn = 'Login';
  static const forgotPassword = 'Ù†Ø³ÙŠØª ÙƒÙ„Ù…Ø© Ø§Ù„Ù…Ø±ÙˆØ±ØŸ';
  static const forgotPasswordEn = 'Forgot Password?';
  static const resetPassword = 'Ø¥Ø¹Ø§Ø¯Ø© ØªØ¹ÙŠÙŠÙ† ÙƒÙ„Ù…Ø© Ø§Ù„Ù…Ø±ÙˆØ±';
  static const resetPasswordEn = 'Reset Password';
  static const sendResetLink = 'Ø¥Ø±Ø³Ø§Ù„ Ø±Ø§Ø¨Ø· Ø§Ù„Ø§Ø³ØªØ¹Ø§Ø¯Ø©';
  static const sendResetLinkEn = 'Send Reset Link';
  static const logout = 'ØªØ³Ø¬ÙŠÙ„ Ø§Ù„Ø®Ø±ÙˆØ¬';
  static const logoutEn = 'Logout';
  static const backToLogin = 'â† Ø§Ù„Ø¹ÙˆØ¯Ø© Ù„ØªØ³Ø¬ÙŠÙ„ Ø§Ù„Ø¯Ø®ÙˆÙ„';
  static const backToLoginEn = 'â† Back to Login';

  // â”€â”€ Navigation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const home = 'Ø§Ù„Ø±Ø¦ÙŠØ³ÙŠØ©';
  static const homeEn = 'Home';
  static const accreditation = 'Ø§Ù„Ø§Ø¹ØªÙ…Ø§Ø¯';
  static const accreditationEn = 'Accreditation';
  static const reports = 'Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ±';
  static const reportsEn = 'Reports';
  static const notifications = 'Ø§Ù„Ø¥Ø´Ø¹Ø§Ø±Ø§Øª';
  static const notificationsEn = 'Notifications';
  static const myAccount = 'Ø­Ø³Ø§Ø¨ÙŠ';
  static const myAccountEn = 'Account';

  // â”€â”€ Dashboard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const dashboard = 'Ø§Ù„ØµÙØ­Ø© Ø§Ù„Ø±Ø¦ÙŠØ³ÙŠØ©';
  static const dashboardEn = 'Dashboard';
  static const completionRate = 'Ø¯Ø±Ø¬Ø© Ø§Ù„Ø§ÙƒØªÙ…Ø§Ù„';
  static const completionRateEn = 'Completion Rate';
  static const uploadedFiles = 'Ø§Ù„Ù…Ù„ÙØ§Øª Ø§Ù„Ù…Ø±ÙÙˆØ¹Ø©';
  static const uploadedFilesEn = 'Uploaded Files';
  static const standards = 'Ø§Ù„Ù…Ø¹Ø§ÙŠÙŠØ±';
  static const standardsEn = 'Standards';
  static const qualityCompliance = 'Ø§Ù„Ø§Ù…ØªØ«Ø§Ù„ Ù„Ù„Ø¬ÙˆØ¯Ø©';
  static const qualityComplianceEn = 'Quality Compliance';
  static const recentActivity = 'Ø¢Ø®Ø± Ø§Ù„Ø£Ù†Ø´Ø·Ø©';
  static const recentActivityEn = 'Recent Activity';

  // â”€â”€ Accreditation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const academicAccreditation = 'Ø§Ù„Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„Ø£ÙƒØ§Ø¯ÙŠÙ…ÙŠ';
  static const academicAccreditationEn = 'Academic Accreditation';
  static const programmaticAccreditation = 'Ø§Ù„Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„Ø¨Ø±Ø§Ù…Ø¬ÙŠ';
  static const programmaticAccreditationEn = 'Programmatic Accreditation';
  static const accreditationSections = 'Ø£Ù‚Ø³Ø§Ù… Ø§Ù„Ø§Ø¹ØªÙ…Ø§Ø¯';
  static const accreditationSectionsEn = 'Accreditation Sections';
  static const requiredDocuments = 'Ø§Ù„Ù…Ù„ÙØ§Øª Ø§Ù„Ù…Ø·Ù„ÙˆØ¨Ø©';
  static const requiredDocumentsEn = 'Required Documents';
  static const uploadFile = 'Ø±ÙØ¹ Ù…Ù„Ù';
  static const uploadFileEn = 'Upload File';
  static const startAnalysis = 'Ø¨Ø¯Ø¡ Ø§Ù„ØªØ­Ù„ÙŠÙ„';
  static const startAnalysisEn = 'Start Analysis';
  static const aiResults = 'Ù†ØªØ§Ø¦Ø¬ Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ø°ÙƒÙŠ';
  static const aiResultsEn = 'AI Analysis Results';
  static const qualityScore = 'Ø¯Ø±Ø¬Ø© Ø§Ù„Ø¬ÙˆØ¯Ø©';
  static const qualityScoreEn = 'Quality Score';
  static const recommendations = 'Ø§Ù„ØªÙˆØµÙŠØ§Øª';
  static const recommendationsEn = 'Recommendations';
  static const fileUploadSuccess = 'ØªÙ… Ø±ÙØ¹ Ø§Ù„Ù…Ù„Ù Ø¨Ù†Ø¬Ø§Ø­';
  static const fileUploadSuccessEn = 'File uploaded successfully';
  static const fileUploadError = 'Ø­Ø¯Ø« Ø®Ø·Ø£ Ø£Ø«Ù†Ø§Ø¡ Ø±ÙØ¹ Ø§Ù„Ù…Ù„Ù';
  static const fileUploadErrorEn = 'Error uploading file';

  // â”€â”€ Deadlines â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const deadlines = 'Ø§Ù„Ù…ÙˆØ§Ø¹ÙŠØ¯ Ø§Ù„Ù†Ù‡Ø§Ø¦ÙŠØ©';
  static const deadlinesEn = 'Deadlines';
  static const setDeadline = 'ØªØ­Ø¯ÙŠØ¯ Ø§Ù„Ù…ÙˆØ¹Ø¯ Ø§Ù„Ù†Ù‡Ø§Ø¦ÙŠ';
  static const setDeadlineEn = 'Set Deadline';
  static const reminder = 'Ø§Ù„ØªØ°ÙƒÙŠØ±Ø§Øª';
  static const reminderEn = 'Reminders';
  static const oneWeekBefore = 'Ù‚Ø¨Ù„ Ø£Ø³Ø¨ÙˆØ¹';
  static const oneWeekBeforeEn = '1 week before';
  static const oneDayBefore = 'Ù‚Ø¨Ù„ ÙŠÙˆÙ…';
  static const oneDayBeforeEn = '1 day before';
  static const onDueDate = 'ÙŠÙˆÙ… Ø§Ù„Ø§Ø³ØªØ­Ù‚Ø§Ù‚';
  static const onDueDateEn = 'On due date';

  // â”€â”€ Reports â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const myReports = 'ØªÙ‚Ø§Ø±ÙŠØ±ÙŠ';
  static const myReportsEn = 'My Reports';
  static const createReport = 'Ø¥Ù†Ø´Ø§Ø¡ ØªÙ‚Ø±ÙŠØ± Ø¬Ø¯ÙŠØ¯';
  static const createReportEn = 'Create New Report';
  static const sendToReviewer = 'Ø¥Ø±Ø³Ø§Ù„ Ù„Ù„Ù…Ø±Ø§Ø¬Ø¹';
  static const sendToReviewerEn = 'Send to Reviewer';
  static const gapAnalysis = 'ØªØ­Ù„ÙŠÙ„ Ø§Ù„ÙØ¬ÙˆØ§Øª';
  static const gapAnalysisEn = 'Gap Analysis';
  static const reviewerNotes = 'Ù…Ù„Ø§Ø­Ø¸Ø§Øª Ø§Ù„Ù…Ø±Ø§Ø¬Ø¹';
  static const reviewerNotesEn = 'Reviewer Notes';
  static const editHistory = 'ØªØ§Ø±ÙŠØ® Ø§Ù„ØªØ¹Ø¯ÙŠÙ„Ø§Øª';
  static const editHistoryEn = 'Edit History';

  // â”€â”€ Chat â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const chat = 'Ø§Ù„Ù…Ø­Ø§Ø¯Ø«Ø§Øª';
  static const chatEn = 'Chat';
  static const reviewer = 'Ø§Ù„Ù…Ø±Ø§Ø¬Ø¹';
  static const reviewerEn = 'Reviewer';
  static const typeMessage = 'Ø§ÙƒØªØ¨ Ø±Ø³Ø§Ù„Ø©...';
  static const typeMessageEn = 'Type a message...';
  static const sendMessage = 'Ø¥Ø±Ø³Ø§Ù„';
  static const sendMessageEn = 'Send';

  // â”€â”€ Profile â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const profile = 'Ø§Ù„Ù…Ù„Ù Ø§Ù„Ø´Ø®ØµÙŠ';
  static const profileEn = 'Profile';
  static const personalInfo = 'Ø§Ù„Ù…Ø¹Ù„ÙˆÙ…Ø§Øª Ø§Ù„Ø´Ø®ØµÙŠØ©';
  static const personalInfoEn = 'Personal Information';
  static const firstName = 'Ø§Ù„Ø§Ø³Ù… Ø§Ù„Ø£ÙˆÙ„';
  static const firstNameEn = 'First Name';
  static const lastName = 'Ø§Ø³Ù… Ø§Ù„Ø¹Ø§Ø¦Ù„Ø©';
  static const lastNameEn = 'Last Name';
  static const saveChanges = 'Ø­ÙØ¸ Ø§Ù„ØªØ¹Ø¯ÙŠÙ„Ø§Øª';
  static const saveChangesEn = 'Save Changes';
  static const changePassword = 'ØªØºÙŠÙŠØ± ÙƒÙ„Ù…Ø© Ø§Ù„Ù…Ø±ÙˆØ±';
  static const changePasswordEn = 'Change Password';
  static const oldPassword = 'ÙƒÙ„Ù…Ø© Ø§Ù„Ù…Ø±ÙˆØ± Ø§Ù„Ø­Ø§Ù„ÙŠØ©';
  static const oldPasswordEn = 'Current Password';
  static const newPassword = 'ÙƒÙ„Ù…Ø© Ø§Ù„Ù…Ø±ÙˆØ± Ø§Ù„Ø¬Ø¯ÙŠØ¯Ø©';
  static const newPasswordEn = 'New Password';
  static const darkMode = 'Ø§Ù„ÙˆØ¶Ø¹ Ø§Ù„Ø¯Ø§ÙƒÙ†';
  static const darkModeEn = 'Dark Mode';
  static const language = 'Ø§Ù„Ù„ØºØ©';
  static const languageEn = 'Language';
  static const arabic = 'Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©';
  static const english = 'English';

  // â”€â”€ Admin â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const employees = 'Ø§Ù„Ù…ÙˆØ¸ÙÙˆÙ†';
  static const employeesEn = 'Employees';
  static const addEmployee = 'Ø¥Ø¶Ø§ÙØ© Ù…ÙˆØ¸Ù Ø¬Ø¯ÙŠØ¯';
  static const addEmployeeEn = 'Add New Employee';
  static const roles = 'Ø§Ù„Ø£Ø¯ÙˆØ§Ø±';
  static const rolesEn = 'Roles';
  static const addRole = 'Ø¥Ù†Ø´Ø§Ø¡ Ø¯ÙˆØ± Ø¬Ø¯ÙŠØ¯';
  static const addRoleEn = 'Create New Role';
  static const permissions = 'Ø§Ù„ØµÙ„Ø§Ø­ÙŠØ§Øª';
  static const permissionsEn = 'Permissions';
  static const pricing = 'Ø§Ù„Ø£Ø³Ø¹Ø§Ø±';
  static const pricingEn = 'Pricing';
  static const activityLog = 'Ø³Ø¬Ù„ Ø§Ù„Ø£Ù†Ø´Ø·Ø©';
  static const activityLogEn = 'Activity Log';
  static const colleges = 'Ø§Ù„ÙƒÙ„ÙŠØ§Øª';
  static const collegesEn = 'Colleges';
  static const subscription = 'Ø§Ù„Ø§Ø´ØªØ±Ø§Ùƒ';
  static const subscriptionEn = 'Subscription';

  // â”€â”€ Common â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  static const save = 'Ø­ÙØ¸';
  static const saveEn = 'Save';
  static const cancel = 'Ø¥Ù„ØºØ§Ø¡';
  static const cancelEn = 'Cancel';
  static const delete = 'Ø­Ø°Ù';
  static const deleteEn = 'Delete';
  static const edit = 'ØªØ¹Ø¯ÙŠÙ„';
  static const editEn = 'Edit';
  static const search = 'Ø¨Ø­Ø«...';
  static const searchEn = 'Search...';
  static const loading = 'Ø¬Ø§Ø±ÙŠ Ø§Ù„ØªØ­Ù…ÙŠÙ„...';
  static const loadingEn = 'Loading...';
  static const noData = 'Ù„Ø§ ØªÙˆØ¬Ø¯ Ø¨ÙŠØ§Ù†Ø§Øª';
  static const noDataEn = 'No data available';
  static const retry = 'Ø¥Ø¹Ø§Ø¯Ø© Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø©';
  static const retryEn = 'Retry';
  static const confirm = 'ØªØ£ÙƒÙŠØ¯';
  static const confirmEn = 'Confirm';
  static const yes = 'Ù†Ø¹Ù…';
  static const yesEn = 'Yes';
  static const no = 'Ù„Ø§';
  static const noEn = 'No';
  static const markAllRead = 'ØªÙ…ÙŠÙŠØ² Ø§Ù„ÙƒÙ„ ÙƒÙ…Ù‚Ø±ÙˆØ¡';
  static const markAllReadEn = 'Mark all as read';
  static const support = 'Ø§Ù„Ø¯Ø¹Ù… Ø§Ù„ÙÙ†ÙŠ';
  static const supportEn = 'Support';
}
