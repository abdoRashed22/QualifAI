...
class _ProfileViewState extends State<_ProfileView> {
  ...
  // Keep last avatar to prevent it from disappearing during cubit.load()
  String? _photoUrl;
      ...
      body: BlocConsumer<ProfileCubit, ProfileState>(
        listener: (ctx, state) {
          if (state is ProfileLoaded) {
            ...
            // cache avatar values to keep showing them while cubit emits loading
            // API returns: profileImage (not photoUrl)
            _photoUrl = state.profile['profileImage'] as String? ??
                state.profile['photoUrl'] as String? ??
                _photoUrl;
            _localPhotoPath =
                state.profile['localPhotoPath'] as String? ?? _localPhotoPath;
          }
                            ...
                            onPressed: () {
                              if (_oldPassCtrl.text.isNotEmpty &&
                                ...
                                cubit.changePassword(
                                    _oldPassCtrl.text, _newPassCtrl.text);
                              }
                            },
                          ),
                        ],
