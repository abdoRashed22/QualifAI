...
class DeadlinesCubit extends Cubit<DeadlinesState> {
  ...
  Future<void> load() async {
    ...
    r.fold((f) => emit(DeadlinesError(f.message)),
        (list) => emit(DeadlinesLoaded(list)));
  }

  void filterBy(String filter) {
    if (state is DeadlinesLoaded) {
      emit(DeadlinesLoaded((state as DeadlinesLoaded).deadlines,
          filter: filter));
    }
  }
  ...
  Future<void> setDeadline(
    ...
    final r = await _repo.setDeadline(
        docId, deadline, remindOneWeek, remindOneDay, remindOnDue);
    r.fold(
      (f) => emit(DeadlinesError(f.message)),
      (_) {
        // Reload deadlines to show the updated data
        load();
      },
    );
  }
