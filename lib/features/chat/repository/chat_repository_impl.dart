...
class ChatRepositoryImpl implements ChatRepository {
  ...
  Future<Either<Failure, List<dynamic>>> getColleges() async {
    try {
      return Right(await _remote.getColleges());
    } on Failure catch (f) {
      return Left(f);
    } catch (_) {
  ...
  Future<Either<Failure, List<dynamic>>> getMessages(int collegeId) async {
    try {
      return Right(await _remote.getMessages(collegeId));
    } on Failure catch (f) {
      return Left(f);
    } catch (_) {
      return const Left(UnknownFailure());
  ...
  Future<Either<Failure, void>> sendMessage(
      String content, int collegeId, int? receiverId) async {
    try {
      await _remote.sendMessage(content, collegeId, receiverId: receiverId);
